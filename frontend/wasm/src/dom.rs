...
pub fn fetch_2d_context(canvas_name: &str) -> CanvasRenderingContext2d {
    ...
    let canvas = canvas_elem
        .dyn_into::<web_sys::HtmlCanvasElement>().expect("canvas element should have converted into HtmlCanvasElement");
...
fn get_element_by_id(doc: &Document, id: &str) -> Element {
    doc.get_element_by_id(id).expect(&format!("document should have a {}", id))
}
